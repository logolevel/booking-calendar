...
export class SettingsService {
  ...
  async getPrimeMemberOpenHour(): Promise<number> {
    const setting = await this.prisma.setting.findUnique({
      where: { key: PRIME_MEMBER_OPEN_HOUR_KEY },
    });
    if (!setting) {
      return PRIME_TIME_MEMBER_OPEN_HOUR;
    }
    const parsed = Number(setting.value);
    return Number.isInteger(parsed) && parsed >= 0 && parsed <= 23
      ? parsed
  ...
  async setPrimeMemberOpenHour(hour: number): Promise<void> {
    const value = String(hour);
    await this.prisma.setting.upsert({
      where: { key: PRIME_MEMBER_OPEN_HOUR_KEY },
      create: { key: PRIME_MEMBER_OPEN_HOUR_KEY, value },
      update: { value },
    });
  }
